...
import org.apis.datasource.Serializers;
...
public class MigrateHeaderSourceTotalDiff implements Runnable {
    ...
    public void run() {
        ...
        if (blockStore.getBestBlock().getNumber() > 0 && blockStore.getChainBlockByNumber(1) == null) {
            ...
            BigInteger totalRewardPoint = blockStore.getChainBlockByNumber(0).getCumulativeRewardPoint();
            for (int i = 1; i < firstFullBlockNum; ++i) {
                totalRewardPoint = totalRewardPoint.add(headerSource.get(i).getRewardPoint());
            }
            blockStore.saveBlock(firstFullBlock, totalRewardPoint.add(firstFullBlock.getRewardPoint()), true);
