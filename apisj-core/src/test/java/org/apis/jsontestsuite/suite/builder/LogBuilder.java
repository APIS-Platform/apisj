...
import org.apis.jsontestsuite.suite.model.LogTck;
...
public class LogBuilder {
    ...
    public static LogInfo build(LogTck logTck){
        ...
        byte[] address = Utils.parseData(logTck.getAddress());
        byte[] data = Utils.parseData(logTck.getData());

        List<DataWord> topics = new ArrayList<>();
        for (String topicTck : logTck.getTopics())
            topics.add(new DataWord(Utils.parseData(topicTck)));
